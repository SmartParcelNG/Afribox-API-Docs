...
- **Dates** are served as `M/d/yyyy h:mm:ss AM/PM` with **no timezone**; the server runs at
  **UTC+1** (Lagos). A parcel created late in the evening in Abidjan may be dated the next day.
  Date **inputs** (e.g. parcel reports) are `MM/DD/YYYY`; `YYYY-MM-DD` is also accepted.
